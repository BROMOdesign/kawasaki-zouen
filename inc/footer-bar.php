...
function enqueue_footer_bar_scripts() {
  wp_enqueue_style( 'beauty-footer-bar', get_template_directory_uri() . '/assets/css/footer-bar.min.css', false, version_num() );
  wp_enqueue_script( 'beauty-footer-bar', get_template_directory_uri() . '/assets/js/footer-bar.min.js', array( 'jquery' ), version_num(), true );
}

function add_footer_bar_styles( $inline_styles, $options ) {

  $inline_styles[] = array(
    'selectors' => '.c-footer-bar',
    'properties' => array(
      sprintf( 'background: rgba(%s, %s)', esc_html( implode( ',', hex2rgb( $options['footer_bar_bg'] ) ) ), esc_html( $options['footer_bar_tp'] ) ),
      sprintf( 'border-color: %s', esc_html( $options['footer_bar_border'] ) )
    )
  );

  $inline_styles[] = array(
    'selectors' => '.c-footer-bar a',
    'properties' => sprintf( 'color: %s', esc_html( $options['footer_bar_color'] ) )
  );

  $inline_styles[] = array(
    'selectors' => '.c-footer-bar__item + .c-footer-bar__item',
    'properties' => sprintf( 'border-color: %s', esc_html( $options['footer_bar_border'] ) )
  );
...
function render_footer_bar() {
  ...
  <div id="js-modal-content" class="c-footer-bar__modal-content u-hidden">
  	<ul class="c-footer-bar__share u-clearfix">
  		...
  		<li class="c-footer-bar__share-btn">
  			<a href="http://b.hatena.ne.jp/entry/" class="hatena-bookmark-button" data-hatena-bookmark-layout="simple" data-hatena-bookmark-width="50" data-hatena-bookmark-height="50" title="このエントリーをはてなブックマークに追加"><img src="<?php echo get_template_directory_uri(); ?>/assets/images/hatenabookmark-logomark.svg" alt="このエントリーをはてなブックマークに追加" width="20" height="20" style="border: none;" /></a><script type="text/javascript" src="https://cdn-ak.b.st-hatena.com/js/bookmark_button.js" charset="utf-8" async="async"></script>
  		</li>
  	</ul>
  </div>
  <?php
